...
- `Resource` / `Channel` / `Buffer`: not implemented (large-file transfer and buffered-channel protocols that build on top of `Link`).
...
In short: a peer running this codebase now produces and validates announces, single-destination encrypted packets (with opt-in delivery proofs), full `Link` handshakes/data exchange, small-payload Request/Response, path requests/responses, and single-packet LXMF messages that a real Reticulum/LXMF node would also consider valid, and the TCP gateway speaks real `TCPInterface` framing. Both single-destination messages and `Link`s (with everything that flows over one) can now be relayed multiple hops through an intermediate peer that owns neither endpoint. What's still missing: `Resource`/`Channel` transfers (so requests, responses, and messages that don't fit in a single packet/link MDU have no fallback), and LXMF's propagation-node/stamp economy. Note also that `Link`'s establishment doesn't currently drive the demo UI (`browser/main.js` messages over single-destination encryption directly) — it's available as a verified API, not yet wired into the chat demo.

## Known limitations

- Both single-destination messages and `Link`s (including everything sent over one, like Request/Response) can now be discovered and relayed across multiple hops through an intermediate peer that owns neither endpoint — see [Compliance](#compliance). Requests, responses, and messages too large for a single packet/link MDU still have no `Resource`-based fallback, so they're limited to what fits in one packet.
