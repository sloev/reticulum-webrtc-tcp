...
- `Link.sendResource()` (`shared/rns/protocol.js`'s "RNS.Resource" section): a chunked-transfer implementation for payloads too large for a single packet, using RNS.Resource's exact wire primitives — the ADV/REQ/part/PROOF packet contexts and `ResourceAdvertisement`'s msgpack field layout (including 1-based segment numbering, and a part size matching real `RNS.Resource`'s own `sdu` exactly, required for a real peer to agree on how many parts a transfer has — this turned out to be a *different*, larger value than `RNS.Link.MDU`, a distinction this project's `RESOURCE_SDU`/`LINK_MDU` constants initially conflated; see below), the same hashmap-based part-identification scheme (parts carry no explicit index; both sides identify one by hashing its content together with a shared random value), and the same completion-proof hash formula. The receiver requests parts in a **rate-adaptive window** matching `RNS.Resource`'s own growth/promotion logic (starts small, grows each fully-satisfied round, promotes its ceiling after sustained fast throughput) — though not yet its retry-driven *shrinking*, since there's no per-part retry/timeout mechanism for a shrink to attach to. A real sender doesn't care how a receiver paces its requests regardless (it just resends whatever's asked for): `test-integration/resource-cross-language-check.mjs` has a real Python `RNS.Resource` correctly reassemble a transfer sent by this stack (including one split across multiple segments), and this stack correctly reassemble a transfer sent by real `RNS.Resource`, including a bz2-compressed one, over a real `RNS.Link`.
  ...
  Payloads bigger than one segment (`RESOURCE_SEGMENT_MAX_SIZE`, sized to fit `RESOURCE_MAX_PARTS` parts in one advertisement — real RNS's own per-segment cap is far larger, 1MiB-1, but a real receiver just processes whatever segment size the sender advertises) are split into multiple segments, sent one at a time, each a fully independent advertise/request/part/proof cycle linked by a shared "original hash" — this makes *sending* an arbitrarily large payload to a real peer work, but *receiving* a segment bigger than `RESOURCE_SEGMENT_MAX_SIZE` from a real sender still doesn't, since that needs RNS's HMU packets (splitting a hashmap that doesn't fit in one packet across several) that aren't implemented here.
