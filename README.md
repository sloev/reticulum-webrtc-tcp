...
This is a from-scratch implementation, not a port or a wrapper of the reference [`rns`](https://github.com/markqvist/Reticulum)/[`lxmf`](https://github.com/markqvist/LXMF) Python packages. [Compliance](#compliance) is a feature-by-feature table with Python/JS references and how each row is verified — byte-exact against captured reference output, or live against a real running Python process. See [Known limitations](#known-limitations) for what's deliberately out of scope.
...
`Reticulum.onPacketReceived` (`shared/rns/index.js`) parses the packet header on every interface and increments hop count on receipt, matching `RNS.Transport.inbound()`. ANNOUNCE packets update a path table (destination hash → lowest hop count + neighbor) and rebroadcast with the incremented hop count; DATA/PROOF addressed to a known local destination or link deliver locally. A DATA/PROOF packet addressed to a non-local destination is forwarded toward the path table's next hop (`Reticulum._forward`, via `sendDataToPeer` when supported); the path-request control destination is answered from the table. Any incoming byte with its high bit (`0x80`) set is relayed unparsed — this is how the TCP gateway bridges arbitrary traffic without RNS framing.

Path discovery is hop-count-aware; single-destination messages, LINKREQUEST packets, and an established `Link`'s ongoing traffic can all relay through an intermediate peer owning neither endpoint (`_forward`/`_forwardLinkRequest`/`_forwardLinkTraffic`), verified functionally with an A↔Relay↔B topology.

**Known limitation** (found via `test-integration/propagation-node-nomadnet-interop-check.mjs`): `_forward`/`_forwardLinkRequest` don't rewrite a packet's `transport_id` when relaying. Real RNS's `Transport.packet_filter()` discards a non-announce packet whose `transport_id` doesn't match the receiver's own transport identity — this only surfaces when relaying a Link handshake between two *other* transport-enabled RNS processes (not when this project's own instance is a Link endpoint, which every other live check is). Not fixed; the workaround is connecting the two real processes directly, or through a node that's an actual endpoint of at least one side.

### TCP gateway and the WebRTC↔TCP bridge

`node/tcp-gateway.js` decodes incoming HDLC frames into the RNS packet stream and frames outgoing traffic the same way, matching `RNS.Interfaces.TCPInterface` (`node/hdlc.js`: `0x7E` delimiter, `0x7D` escape, byte-exact tested) — this is what lets an unmodified Reticulum node join over its standard `TCPClientInterface`; every live check spawning a real Python process talks to this project through this gateway.

A single Node.js process (`node/index.js`) runs both a WebRTC interface and this TCP gateway on one `Reticulum` instance: a packet arriving on either is handled by the same `onPacketReceived`/`_forward`/`_broadcastExcept` logic regardless of source or destination interface — the same relaying mechanism any two-interface node uses, just with dissimilar interfaces on each side. This is how a browser-only WebRTC peer and a real Python `rns` process over TCP reach each other.

What the bridge doesn't do: no IFAC or other authentication on either interface — whoever connects is trusted the same way an unauthenticated `TCPServerInterface`/`AutoInterface` would be in real Reticulum.
...
| Real end-user client ↔ real propagation node, through this project | n/a | `test-integration/propagation-node-nomadnet-interop-check.mjs` | DONE — both directions verified live and repeatably against a real, unmodified `LXMRouter` propagation node and a real, unmodified `nomadnet` client (JS-upload/NomadNet-sync, and NomadNet-send/JS-download via `LXMRouter.handle_outbound()`/`process_deferred_stamps()`/`LXStamper`) — see `compliance.md`'s Part 1 writeup | live |
...
- State (identity cache, path table, links, propagation storage) is in-memory only by default and lost on restart — except in Node, where `RNS_STORAGE_DIR` persists the identity cache, path table, retained ratchets, and `PropagationNode`'s store to disk (see [Compliance](#compliance)). Links themselves are never persisted; the browser has no equivalent beyond the demo's own per-session identity caching.
