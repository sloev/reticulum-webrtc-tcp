...
- `Resource`'s full rate-adaptive windowed transfer: implemented with a fixed-size request window (rather than one that ramps up on a fast link) and no *outgoing* compression, and interoperates with a real `RNS.Resource` peer in both directions, including multi-segment transfers and a real peer's compressed data (see [Compliance](#compliance)). `Channel`/`Buffer` (`shared/rns/channel.js`, `shared/rns/buffer.js`) *are* implemented, including the RTT-adaptive send window, and also interoperate with real `RNS.Channel`/`RNS.Buffer` in both directions, compression included (see [Compliance](#compliance)).
