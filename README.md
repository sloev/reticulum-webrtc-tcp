...
- LXMF beyond OPPORTUNISTIC delivery and this project's own store-and-forward: no DIRECT (link-based) delivery method, no compression negotiation, and no announce-encoded stamp-cost/capacity advertising. A message from this stack can now reach a real propagation node and be synced onward to another real node as a peer (see above), but two-way interop with an official end-user LXMF client (Sideband, NomadNet) as the message's actual sender/recipient hasn't been tested.
